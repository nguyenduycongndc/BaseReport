...
namespace WebReport.Controllers
{
    ...
    public class ReportController : BaseController
    {
        ...
        public ReportController(MyDbContext dbContext, IUnitOfWork uow, IConfiguration config, IWebHostEnvironment webHostEnvironment, ITokenService tokenService, ILogger<ReportController> logger) : base(config, tokenService)
        {
        ...
        public IActionResult Search(string jsonData)
        {
            try
            {
                ...
                if (obj.StartNumber >= 0 && obj.PageSize > 0)
                {
                    dataNew = arrData.Skip(obj.StartNumber).Take(obj.PageSize);
                }
                return Ok(new { code = "1", msg = "success", data = dataNew, total = arrData.Count() });
